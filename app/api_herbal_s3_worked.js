...
const { IncomingForm, Fields, Files, Options } = require('formidable');
...
const fs = require('fs');
const { s3Client, PutObjectCommand } = require('../config/s3-instance')
...
// let form = formidable({ multiples: false }); // รับไฟล์เดียว
// const form = formidable({}); // รับไฟล์เดียว
// const options = {
//   multiples: false,
...
// }
  ...
  try {
    ...
    if (cleanedFields) {
          console.log('req cleanedFields',cleanedFields)
          // console.log('JSON.stringify fields', JSON.stringify(fields, null, 2))
          result = await herbal.create(cleanedFields);
        }
    ...
    const fileStream = fs.createReadStream(file.filepath);
      ...
      const command = new PutObjectCommand(params);
        ...
        const updateResult = await herbal.update(
        ...
        res.json({
          ...
          message: "create herbal (result) not successfull"
        });        
      }
    ...
    console.error("Error in /herbal add:", error);
